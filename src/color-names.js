...
const colorNames = {
  init: function () {
    let color, rgb, hsl;
    for (let i = 0; i < colorNames.names.length; i++) {
      color = "#" + colorNames.names[i][0];
      rgb = colorNames.rgb(color);
      hsl = colorNames.hsl(color);
      colorNames.names[i].push(rgb[0], rgb[1], rgb[2], hsl[0], hsl[1], hsl[2]);
    }
  },
  ...
  name: function (color) {
    ...
    let rgb = colorNames.rgb(color);
    let r = rgb[0],
      g = rgb[1],
      b = rgb[2];
    let hsl = colorNames.hsl(color);
    let h = hsl[0],
      s = hsl[1],
      l = hsl[2];
    let ndf1 = 0;
    ...
    for (let i = 0; i < colorNames.names.length; i++) {
      if (color === "#" + colorNames.names[i][0])
        return ["#" + colorNames.names[i][0], colorNames.names[i][1], true];

      ndf1 =
        Math.pow(r - colorNames.names[i][2], 2) +
        Math.pow(g - colorNames.names[i][3], 2) +
        Math.pow(b - colorNames.names[i][4], 2);
      ndf2 =
        Math.pow(h - colorNames.names[i][5], 2) +
        Math.pow(s - colorNames.names[i][6], 2) +
        Math.pow(l - colorNames.names[i][7], 2);
      ndf = ndf1 + ndf2 * 2;
      if (df < 0 || df > ndf) {
        df = ndf;
        cl = i;
    ...
    return cl < 0
      ...
      : ["#" + colorNames.names[cl][0], colorNames.names[cl][1], false];
  },
