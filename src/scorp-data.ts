...
import { MarketplaceListings } from './App'
import { passesFilters, FilteredScorpionsProps } from './FilteredScorpions'

const metadata = require('./metadata.json')
const scrops: { [id: string]: ScorpMetadata } = metadata.entries
const rareAttributes = require('./raretraits.json')

export function getFloorPrice(listings: MarketplaceListings, filteredScorpionProps: FilteredScorpionsProps) {
  let lowestPrice = Number.MAX_SAFE_INTEGER
  let lowestPriceText = ''
  let lowestPriceLink = 'https://rarechest.io/collections/scorpions/'
  if (listings.results) {
    for (const listing of listings.results) {
      let sid = listing.series_no.toString()

      while (sid.length !== 4) {
        sid = '0' + sid
      }
      if (passesFilters(sid, filteredScorpionProps)) {
        for (const ask of listing.live_asks) {
          if (parseFloat(ask.amt) < lowestPrice) {
            lowestPrice = parseFloat(ask.amt)
            lowestPriceLink = `https://rarechest.io/collections/scorpions/${listing.series_no}`
          }
        }
      }
    }
    ...
    lowestPriceText =
      lowestPrice === Number.MAX_SAFE_INTEGER ? 'Floor price: ??? (None for sale)' : `Floor price: ${lowestPrice} XRD`
  }

  return { lowestPriceText, lowestPriceLink }
}
...
export function getRarity(scorpId: string): RarityProfile {
  ...
  for (const att of Object.keys(scorpAttributes) as (keyof Attributes)[]) {
    ...
    if (rareAttributes[att][attributeValueForScorp]) {
      rarityProfile.rarity += parseInt(rareAttributes[att][attributeValueForScorp].rarity.substring(0, 1))
...
export function prettyFormatColor(color: string): string {
  ...
  prettyColor = prettyColor.replace(new RegExp('\\b(' + expStr + ')\\b', 'gi'), ' ').replace(/\s{2,}/g, ' ')
...
export function isBaller(scorpId: string): boolean {
  const scorpMetadata = getScorpMetadata(scorpId).attributes
  return scorpMetadata.claw_left === 'ball' && scorpMetadata.claw_right === 'ball' && scorpMetadata.tail === 'ball'
}
